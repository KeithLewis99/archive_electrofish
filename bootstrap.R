...
write.csv(df_sum, "data_derived/boot/SB_disagg_2001_2003.csv", row.names = F)
...
write.csv(sb.boot.ci, "data_derived/boot/SB_boot_site_2001_2003.csv", row.names = F)
...
write.csv(sb.boot.year.ci, "data_derived/boot/SB_boot_yr_2001_2003.csv", row.names = F)
...
df_tp_spp_yr_stn <- df_tp |>
   ...
   summarise(bio.sum = sum(Weight.g, na.rm = T), abun = n()) |>
   mutate(abun.stand = abun/Area*100, bio.stand = bio.sum/Area*100)
write.csv(df_tp_spp_yr_stn, "data_derived/boot/TP_disagg_2012_2018.csv", row.names = F)
...
write.csv(tp.boot.ci, "data_derived/boot/TP_boot_site_2012_2018.csv", row.names = F)
...
tp.year.boot.ci <- full_join(spp.den.tp.year.boot.ci, spp.bio.tp.year.boot.ci, by = c("Species")) |>
   rename(density = mean.x, den.ll = ll.x, den.ul = ul.x,
          biomass = mean.y, bio.ll = ll.y, bio.ul = ul.y)
tp.year.boot.ci
write.csv(tp.year.boot.ci, "data_derived/boot/TP_boot_yr_2012_2018.csv", row.names = F)
...
df_hl_spp_yr_stn <- df_hl |>
   ...
   summarise(bio.sum = sum(Weight.g, na.rm = T), abun = n()) |>
   mutate(abun.stand = abun/Area*100, bio.stand = bio.sum/Area*100)
write.csv(df_hl_spp_yr_stn, "data_derived/boot/HL_disagg_2013_2019.csv", row.names = F)
...
write.csv(hl.boot.ci, "data_derived/boot/hl_boot_site_2012_2018.csv", row.names = F)
...
hl.year.boot.ci <- full_join(spp.den.hl.year.boot.ci, spp.bio.hl.year.boot.ci, by = c("Species")) |>
   rename(density = mean.x, den.ll = ll.x, den.ul = ul.x,
          biomass = mean.y, bio.ll = ll.y, bio.ul = ul.y)
hl.year.boot.ci
write.csv(hl.year.boot.ci, "data_derived/boot/hl_boot_yr_2012_2018.csv", row.names = F)
